...
import { Si_sesionsAddModalComponent } from './si_sesions-add-modal/si_sesions-add-modal.component';
...
@Component({
selector: 'si_sesions-table',
templateUrl: './si_sesions-table.html',
styleUrls: ['./si_sesions-table.scss'],
})
export class Si_sesionsTableComponent implements OnInit {
    data;
    backpage: boolean;
      ...
      if (this.user.super) {
        ...
        if (userModules[0]) {
          for (const element in userModules) {
            if (userModules[element].path === '/pages/si_sesions') {
    ...
    private findByIdSi_user(id: number): void {
      this.service
        ...
        .pipe(take(1))
        .subscribe(
            (data: Si_sesionsResponseInterface) => {
                if (data.success) {
                  this.data = new MatTableDataSource<Si_sesionsInterface>(data.result);
    ...
    editModalShow(si_sesions: Si_sesionsInterface) {
      const dialogRef = this.dialog.open(Si_sesionsAddModalComponent, {
              width: '550px',
              data: si_sesions
          });
    ...
    onDeleteConfirm(event, item): void {
      if (window.confirm('¿Estas seguro de querer eliminar este registro?')) {
          this.service.remove(item.idsi_sesion)
          .pipe(take(1))
          .subscribe(
