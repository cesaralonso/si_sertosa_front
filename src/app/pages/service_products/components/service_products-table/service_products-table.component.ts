...
import { Service_productsAddModalComponent } from './service_products-add-modal/service_products-add-modal.component';
...
@Component({
selector: 'service_products-table',
templateUrl: './service_products-table.html',
styleUrls: ['./service_products-table.scss'],
})
export class Service_productsTableComponent implements OnInit {
    data;
    backpage: boolean;
      ...
      if (this.user.super) {
        ...
        if (userModules[0]) {
          for (const element in userModules) {
            if (userModules[element].path === '/pages/service_products') {
    ...
    private findByIdProduct(id: number): void {
      this.service
        ...
        .pipe(take(1))
        .subscribe(
            (data: Service_productsResponseInterface) => {
                if (data.success) {
                  this.data = new MatTableDataSource<Service_productsInterface>(data.result);
    ...
    addModalShow() {
      const dialogRef = this.dialog.open(Service_productsAddModalComponent, {
              width: '550px',
              data: {}
          });
    ...
    editModalShow(service_products: Service_productsInterface) {
      const dialogRef = this.dialog.open(Service_productsAddModalComponent, {
              width: '550px',
    ...
    onDeleteConfirm(event, item): void {
      if (window.confirm('¿Estas seguro de querer eliminar este registro?')) {
          this.service.remove(item.idservice_product)
          .pipe(take(1))
          .subscribe(
