...
import { Si_alertasAddModalComponent } from './si_alertas-add-modal/si_alertas-add-modal.component';
...
@Component({
selector: 'si_alertas-table',
templateUrl: './si_alertas-table.html',
styleUrls: ['./si_alertas-table.scss'],
})
export class Si_alertasTableComponent implements OnInit {
    data;
    backpage: boolean;
      ...
      if (this.user.super) {
        ...
        if (userModules[0]) {
          for (const element in userModules) {
            if (userModules[element].path === '/pages/si_alertas') {
    ...
    private findByIdSi_user(id: number): void {
      this.service
        ...
        .pipe(take(1))
        .subscribe(
            (data: Si_alertasResponseInterface) => {
                if (data.success) {
                  this.data = new MatTableDataSource<Si_alertasInterface>(data.result);
    ...
    editModalShow(si_alertas: Si_alertasInterface) {
      const dialogRef = this.dialog.open(Si_alertasAddModalComponent, {
              width: '550px',
              data: si_alertas
          });
    ...
    onDeleteConfirm(event, item): void {
      if (window.confirm('¿Estas seguro de querer eliminar este registro?')) {
          this.service.remove(item.idsi_alerta)
          .pipe(take(1))
          .subscribe(
