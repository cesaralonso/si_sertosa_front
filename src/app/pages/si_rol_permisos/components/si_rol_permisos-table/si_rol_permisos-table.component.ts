...
import { Si_rol_permisosAddModalComponent } from './si_rol_permisos-add-modal/si_rol_permisos-add-modal.component';
...
@Component({
selector: 'si_rol_permisos-table',
templateUrl: './si_rol_permisos-table.html',
styleUrls: ['./si_rol_permisos-table.scss'],
})
export class Si_rol_permisosTableComponent implements OnInit {
    data;
    backpage: boolean;
    ...
    displayedColumns: string[] = [
      ...
      'si_permiso_si_permiso_idsi_permiso',
    ];
      ...
      if (this.user.super) {
        ...
        if (userModules[0]) {
          for (const element in userModules) {
            if (userModules[element].path === '/pages/si_rol_permisos') {
    ...
    refill() { 
      this.routeParamsSubs = this.route.params.subscribe(params => {
        if (params['idsi_permiso'] !== undefined) {
          const idsi_permiso = +params['idsi_permiso'];
          this.findByIdSi_permiso(idsi_permiso);
          this.backpage = true;
        }
    ...
    private findByIdSi_permiso(id: number): void {
      this.service
        .findByIdSi_permiso(id)
        .pipe(take(1))
        .subscribe(
            (data: Si_rol_permisosResponseInterface) => {
                if (data.success) {
                  this.data = new MatTableDataSource<Si_rol_permisosInterface>(data.result);
    ...
    editModalShow(si_rol_permisos: Si_rol_permisosInterface) {
      const dialogRef = this.dialog.open(Si_rol_permisosAddModalComponent, {
              width: '550px',
              data: si_rol_permisos
          });
          dialogRef.afterClosed()
              .pipe(take(1))
              .subscribe(data => {
    ...
    onDeleteConfirm(event, item): void {
      if (window.confirm('¿Estas seguro de querer eliminar este registro?')) {
          this.service.remove(item.idsi_rol_permiso)
          .pipe(take(1))
          .subscribe(
