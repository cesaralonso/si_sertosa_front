...
export class Si_usersService {
    ...
    constructor(
       ...
       findByIdCompanyunits = ( id ) : Observable<Si_usersResponseInterface> => {
       ...
       allFromTo = ( fechaDesde: string, fechaHasta: string ) : Observable<Si_usersResponseInterface> => {
       ...
       update = ( si_user: Si_usersInterface ) : Observable<Si_usersResponseInterface> => {
           return this._http.patch<HttpResponse<any>>(this.endPoint, si_user, this.options)
               .pipe(
                   map((response: any) => response),
                   catchError(this.handleError));
       }
       ...
       insert = ( si_user: Si_usersInterface ) : Observable<Si_usersResponseInterface> => {
           return this._http.post<HttpResponse<any>>(this.endPoint, si_user, this.options)
               .pipe(
                   map((response: any) => response),
                   catchError(this.handleError));
       }
       ...
       insertFile = ( si_user: Si_usersInterface, documento?: File ) : Observable<any> => {
           var formData: any = new FormData();
           if (documento) {
               formData.append("documento", documento);
           }
           formData.append("nombre", si_user.nombre);
           formData.append("apmat", si_user.apmat);
           formData.append("appat", si_user.appat);
           formData.append("email", si_user.email);
           formData.append("password", si_user.password);
           formData.append("si_rol_idsi_rol", si_user.si_rol_idsi_rol);
           formData.append("companyunits_idcompanyunits", si_user.companyunits_idcompanyunits);
