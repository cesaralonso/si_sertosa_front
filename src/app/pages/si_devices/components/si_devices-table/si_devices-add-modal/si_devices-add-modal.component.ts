...
import { Si_devicesService } from './../si_devices.service';
...
@Component({
  ...
  styleUrls: [('./si_devices-add-modal.component.scss')],
  templateUrl: './si_devices-add-modal.component.html'
})
export class Si_devicesAddModalComponent implements OnInit {
  ...
  constructor(
    private service: Si_devicesService,
    ...
    public dialogRef: MatDialogRef<Si_devicesAddModalComponent>,
    @Inject(MAT_DIALOG_DATA) public item: Si_devicesInterface
  ) {
    this.form = fb.group({
  ...
  onUpdate(): void {
      if (this.form.valid) {
          this.service
              .update({
                  idsi_device: this.item.idsi_device,
